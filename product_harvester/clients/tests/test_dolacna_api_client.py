...
from product_harvester.clients.usetri_api_client import (
    UsetriClient,
    UsetriAPICategory,
    UsetriAPIProduct,
    UsetriAPIProductDetail,
)


class TestUsetriClient(TestCase):
    def setUp(self):
        self._token = "test_token"
        self._client = UsetriClient(token=self._token)
        self._imported_product = UsetriAPIProduct(
    ...
    @patch("product_harvester.clients.usetri_api_client.requests.Session.post")
    def test_import_product_success(self, mock_post):
        ...
        mock_post.assert_called_once_with(
            UsetriClient._products_endpoint, json=self._imported_product_json, headers={"user-id": self._token}
        )
    ...
    @patch("product_harvester.clients.usetri_api_client.requests.Session.get")
    def test_get_categories_success(self, mock_get):
        ...
        for i in range(2):  # Loop to validate caching
            ...
            want_categories = [UsetriAPICategory(id=1, name="food"), UsetriAPICategory(id=2, name="drink")]
            self.assertEqual(categories, want_categories)
        mock_get.assert_called_once_with(UsetriClient._categories_endpoint)
