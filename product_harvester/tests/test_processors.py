...
class TestPriceTagImageProcessor(TestCase):
    ...
    def test_process_batch_success(self):
        ...
        fake_model = FakeMessagesListChatModel(responses=mock_responses)
        products = PriceTagImageProcessor(fake_model).process_batch(encoded_images=["<image1>", "<image2>", "<image3>"])
    ...
    def test_process_batch_invalid_products_output_json(self):
        ...
        mock_responses = self._make_mock_responses(mock_valid_product.model_dump_json(), "{wat")
        fake_model = FakeMessagesListChatModel(responses=mock_responses)
        products = PriceTagImageProcessor(fake_model).process_batch(encoded_images=["<image1>", "<image2>"])
        self.assertEqual(products, [mock_valid_product])

    def test_process_batch_incomplete_product_output(self):
        mock_valid_product = Product(name="Banana", price=3.45, qty=1, qty_unit="kg")
        # language=JSON
        mock_responses = self._make_mock_responses('{"name":"Bread"}', mock_valid_product.model_dump_json())
        fake_model = FakeMessagesListChatModel(responses=mock_responses)
        products = PriceTagImageProcessor(fake_model).process_batch(encoded_images=["<image1>", "<image2>"])
        self.assertEqual(products, [mock_valid_product])
